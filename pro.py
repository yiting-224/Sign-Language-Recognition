...
def process(num, image):
    ...
    for (i, c) in enumerate(cnts):
        ...
        print("Image #{} -- aspect_ratio={:.2f}, extent={:.2f}, solidity={:.2f}, area={:.2f}".format(num, aspectRatio, extent, solidity, Area))
        print(shape)
        ...
        if num == "Y" or num == "Z":
            ...
            a = 1
        resized = imutils.resize(clone, width=image.shape[1] * a, inter=cv2.INTER_LINEAR)
        cv2.imshow(f"{num} Image", resized)
        cv2.waitKey(0)
...
# cv2.destroyAllWindows()
image = cv2.imread(f"img/Y.png")
process("Y", image)
image = cv2.imread(f"img/Z.png")
process("Z", image)
for i in w_img:
    a = f"{i}.png"
    image = cv2.imread(f"img/{a}")
